...
export default function ThoughtsPage() {
  ...
  const fetchThoughts = useCallback(async () => {
    const uid = getUserId();
    const { data } = await supabase.from("thoughts").select("*").eq("user_id", uid)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("thoughts").insert({
        user_id: uid, text: input.trim(), date: new Date().toISOString().slice(0, 10),
      });
      if (error) throw error;
      setInput(""); toast.success("想法已记录"); fetchThoughts();
  ...
  const deleteThought = async (id: string) => {
    try {
      ...
      toast.success("已删除"); fetchThoughts(); setEditingId(null);
  ...
  const promoteToTodo = async (thought: Thought) => {
    try {
      const uid = getUserId();
      await supabase.from("todos").insert({ user_id: uid, text: thought.text.slice(0, 100), source: "thought", thought_id: thought.id });
  ...
  return (
    <div className="space-y-4">
      ...
      <button onClick={handleSubmit} disabled={loading || !input.trim()}
        className="w-full bg-[#f5c2e7] text-[#1e1e2e] py-2 rounded-lg text-sm font-medium disabled:opacity-40">
        {loading ? "..." : "记下"}
      </button>
      ...
      <div className="space-y-6">
        ...
        {Object.entries(grouped).sort(([a], [b]) => b.localeCompare(a)).map(([date, items]) => {
          const dateLabel = new Date(date).toLocaleDateString("zh-CN", { month: "short", day: "numeric", weekday: "short" });
          return (
            <div key={date}>
              ...
              <div className="space-y-2">
                {items.map((t) => (
                  <div key={t.id}>
                    <div className="bg-[#313244] rounded-lg p-3 group cursor-pointer"
                      onClick={() => editingId === t.id ? setEditingId(null) : startEdit(t)}>
                      {editingId === t.id ? (
                        <div onClick={(e) => e.stopPropagation()} className="space-y-2">
                          <textarea value={editText} onChange={(e) => setEditText(e.target.value)}
                            className="w-full bg-[#1e1e2e] rounded px-2 py-1 text-sm text-[#cdd6f4] outline-none resize-none" rows={3} autoFocus />
                          <div className="flex gap-2 justify-end">
                            <button onClick={() => deleteThought(t.id)} className="text-[10px] text-[#f38ba8] px-2 py-1">删除</button>
                            <button onClick={() => setEditingId(null)} className="text-[10px] text-[#6c7086] px-2 py-1">取消</button>
                            <button onClick={saveEdit} className="text-[10px] bg-[#f5c2e7] text-[#1e1e2e] rounded px-3 py-1 font-medium">保存</button>
                          </div>
                        </div>
                      ) : (
                        <>
                          ...
                          <div className="flex items-center justify-between mt-2">
                            ...
                            {!t.promoted_to_todo && (
                              <button onClick={(e) => { e.stopPropagation(); promoteToTodo(t); }}
                                className="text-[10px] text-[#a6adc8] hover:text-[#f5c2e7] opacity-0 group-hover:opacity-100 transition-opacity">转为待办</button>
                            )}
